    ...
    try:
        ...
        if op.type == 5:
            if wait["autoAdd"] == True:
                ...
                kt.findAndAddContactsByMid(op.param1)
                if (wait["message"] in [""," ","\n",None]):
                    pass
                else:
              ...
              if msg.from_ in admin:
                ...
                for g in gs.members:
                    if _nametarget == g.displayName:
                       ...
                       kt.sendText(g.mid,"HƛHƛHƛ ƊƖ ƧƤƛM  😂")
...
#==================================================================
            ...
            elif "Team on @" in msg.text:
                if msg.from_ in admin:
                    _name = msg.text.replace("Team on @","")
                    _nametarget = _name.rstrip(' ')
                    gs = cl.getGroup(msg.to)
                    targets = []
                    for g in gs.members:
                    ...
                    if targets == []:
                        ...
                        for target in targets:
                            try:
                                team.append(target)
                                cl.sendText(msg.to,"succes add to teamlist")
                            except:
                                pass
                    print "[Command]team add executed"
                else:
                    cl.sendText(msg.to,"Command denied.")
                    cl.sendText(msg.to,"owner permission required.")
            elif msg.text.lower() == 'team list':
              if msg.from_ in admin:
                if admin == []:
                       cl.sendText(msg.to,"The team list empty")
                else:
                    ...
                    if targets == []:
                        ...
                        for target in targets:
                            try:
                                admin.remove(target)
                                team.remove(target)
                                cl.sendText(msg.to,"Succes remove from list")
                            except:
                                pass
